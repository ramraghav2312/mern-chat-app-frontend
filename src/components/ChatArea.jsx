...
import { FiSend, FiInfo, FiMessageCircle, FiEdit2, FiTrash2 } from "react-icons/fi";
...
const ChatArea = ({ selectedGroup, socket, setSelectedGroup }) => {
  ...
  const isAdmin = selectedGroup?.admin?._id === currentUser?._id;
  ...
  // Info button click — modal open karo aur fields set karo
  ...
  const handleEditGroup = async () => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${apiURL}/api/groups/${selectedGroup?._id}`,
  ...
  const handleDeleteGroup = async () => {
    ...
    try {
      const token = currentUser.token;
      await axios.delete(`${apiURL}/api/groups/${selectedGroup?._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast({
  ...
  const renderTypingIndicator = () => {
    ...
    return Array.from(typingUsers).map((username) => (
      <Box
        key={username}
        alignSelf={username === currentUser?.username ? "flex-end" : "flex-start"}
        maxW="70%"
      >
        <Flex
          align="center"
  ...
  return (
    <Flex h="100%" position="relative" direction={{ base: "column", lg: "row" }}>
      <Box
        flex="1"
        display="flex"
      ...
      >
        {selectedGroup ? (
                ...
                >
                  <Flex direction="column" gap={1}>
                    <Flex
                      align="center"
                      mb={1}
                      justifyContent={message.sender._id === currentUser?._id ? "flex-start" : "flex-end"}
                      gap={2}
                    >
          ...
          <Flex h="100%" direction="column" align="center" justify="center" p={8} textAlign="center">
            ...
            <Text color="gray.500">Select a group from the sidebar to start chatting</Text>
          </Flex>
        )}
      </Box>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody pb={4}>
            {/* Group Info — sabko dikhta hai */}
            <Box mb={4} p={3} bg="gray.50" borderRadius="lg">
              <Text fontSize="sm" color="gray.500" mb={1}>Group Name</Text>
              <Text fontWeight="bold">{selectedGroup?.name}</Text>
              <Text fontSize="sm" color="gray.500" mt={2} mb={1}>Description</Text>
              <Text>{selectedGroup?.description}</Text>
              <Text fontSize="sm" color="gray.500" mt={2} mb={1}>Admin</Text>
              <Flex align="center" gap={2}>
                <Avatar size="xs" name={selectedGroup?.admin?.username} />
                <Text fontWeight="medium">{selectedGroup?.admin?.username}</Text>
                <Badge colorScheme="blue" variant="subtle">Admin</Badge>
              </Flex>
            </Box>

            {/* Edit fields — sirf admin ko dikhta hai */}
            {isAdmin && (
              <Box>
                <Text fontWeight="semibold" mb={3} color="gray.700">Edit Group</Text>
                <FormControl mb={3}>
